...
def preprocess_fbi_data(fbi_data: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    religious_biases = ['Anti-Jewish', 'Anti-Protestant', 'Anti-Catholic', 'Anti-Islamic',
                        'Anti-Hindu', 'Anti-Sikh', 'Anti-Buddhist', 'Anti-Church']
    hate_crime_data = fbi_data[
...
def incidents_by_state_total(hate_crime_data: pd.DataFrame):
    ...
    Args:
        hate_crime_data: Filtered religious hate crime data.
    """
    state_incidents = hate_crime_data.groupby(
        'state_name').size().reset_index(name='incident_count')
    fig = px.bar(state_incidents, x='state_name', y='incident_count',
...
def top_5_cities_trends(hate_crime_data: pd.DataFrame):
    ...
    top_cities = hate_crime_data['pug_agency_name'].value_counts().head(
        5).index
    top_cities_data = hate_crime_data[hate_crime_data['pug_agency_name'].isin(
        top_cities)]
    trends_by_city = top_cities_data.groupby(
        ['data_year', 'pug_agency_name']).size().reset_index(name='incident_count')
    fig = px.line(trends_by_city, x='data_year', y='incident_count', color='pug_agency_name',
